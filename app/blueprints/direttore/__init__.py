...
@direttore_auth_required
def dashboard():
    direttore = db.session.query(Direttore).where(Direttore.id == session["direttore"]["id"]).first()
    if not direttore:
    ...
    bancari = sorted(bancari, key=lambda b: len(list(b.richieste_conti_correnti)), reverse=True)
    
    most_spending_clients = get_most_spending_clients_for_filiale(filiale.id) # type: ignore
...
def accetta_prestito():
    ...
    prestito = db.session.query(Prestito).where(Prestito.id == prestito_id).first()
    ...
    if prestito:
        ...
        conto_corrente = db.session.query(ContoCorrente).where(ContoCorrente.id == prestito.conto_corrente_id).first()
        ...
        if accettata:
            ...
            transazione.descrizione = 'Prestito n. ' + str(prestito.id) # type: ignore
...
def garanzia():
    ...
    garanzia = db.session.query(Garanzia).where(Garanzia.id == garanzia_id).first()
